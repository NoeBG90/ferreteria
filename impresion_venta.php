...
$conexio =   conectar_bd();
$query = "select folio_operacion ,vigencia_operacion , tiempo_entrega ,consideraciones , total , subtotal ,iva,iva_porcentual,
            e.nombre as empleado,c2.nombre as cliente ,c2.nom_contacto ,c.fecha_registro ,c2.telefono ,c.metodo_pago,
                concat((case  when length(calle) > 1 then concat('Calle: ', calle, ' ') else '' end),
                (case  when length(exterior) > 1 then concat('Num Ext: ', exterior, ' ') else '' end),
                (case  when length(interior) > 1 then concat('Num Int: ', interior, ' ') else '' end),
                (case  when length(colonia) > 1 then concat('Col: ', colonia, ' ') else '' end),
                (case  when length(cp) > 1 then concat('CP: ', cp, ' ') else '' end),
                (case  when length(ciudad) > 1 then ciudad else '' end)) as direccion
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(80);
        // Título
        $this->Cell(30, 10, iconv('utf-8', 'cp1252', 'Venta'), 0, 0, 'C');
        // Salto de línea
        $this->Ln(30);
    }
...
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Folio: ' . $folio_cotizacion), 0, 1, 'R');
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Fecha: ' . $fecha_registro), 0, 1, 'R');
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', "Atendió: " . $empleado), 0, 1, 'R');
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Consideraciones: ' . $consideraciones), 0, 1, 'R');
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'T. Entrega: ' . $tiempo_entrega), 0, 1, 'R');
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Método Pago: ' . $metodo_pago), 0, 1, 'R');

$pdf->Ln(10);
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Cliente: ' . $cliente), 0, 1);
$pdf->MultiCell(0, 6, iconv('utf-8', 'cp1252', "Dirección: " . $direccion), 0, 1);
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', "Teléfono: " . $telefono), 0, 1);
$pdf->Cell(0, 6, iconv('utf-8', 'cp1252', 'Vendedor: ' . $vendedor), 0, 1);
