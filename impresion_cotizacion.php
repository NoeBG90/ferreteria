...
$query = "select folio_operacion ,vigencia_operacion , tiempo_entrega ,consideraciones , 
                total , subtotal ,iva,iva_porcentual , e.nombre as empleado,c2.nombre as cliente ,c2.nom_contacto ,c.fecha_registro ,c2.telefono,
                c.metodo_pago, concat((case when length(calle) > 1 then concat('Calle: ', calle, ' ') else '' end), 
                ...
                (case when length(cp) > 1 then concat('CP: ', cp, ' ') else '' end), (case when length(ciudad) > 1 then ciudad else '' end)) as direccion
...
$queryvendedor = "select e.* from clientes c,empleados e,operaciones c2 where
            c.id_empleado =e.id_empleado and c2.id_cliente =c.id_cliente 
...
$querydetalle = "select p.producto, p.descripcion,dc.cantidad ,dc.precio ,dc.descuento ,dc.subtotal, p.unidad_medida 
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(80);
        // Título
        $this->Cell(30, 10, iconv('utf-8', 'cp1252', 'Cotización'), 0, 0, 'C');
        // Salto de línea
        $this->Ln(30);
    }
...
$pdf->Cell(0, 7, iconv('utf-8', 'cp1252', "Cotizó: " . $empleado), 0, 1, 'R');
...
$pdf->MultiCell(0, 7, iconv('utf-8', 'cp1252', "Dirección: " . $direccion), 0, 1);
