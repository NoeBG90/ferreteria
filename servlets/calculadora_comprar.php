...
$result=$conexio->query($query);
  $posiciones=sizeof($_SESSION['productos']);
   while($fila=$result->fetch_assoc()){
     ...
     $objdetalle->fila=
     "<tr><td class='id' >".count($_SESSION['productos'])."</td><td >".$fila['SKU']
     ."</td><td >".$fila['producto'].
     "</td><td >
     <input type='text' class='cantidades col-sm-9 text-left' name='txtcantidad'  id='txtcantidad".count($_SESSION['productos'])."' >
     </td><td >
     <input type='text' name='txtprecioventa' id='txtprecioventa".count($_SESSION['productos'])."' class='precios col-sm-9 text-left' value='".$fila['precio_compra']."' readonly>
     </td><td >
...
//print_r($_SESSION['productos']);
  for($i=0;$i<sizeof($_SESSION['productos']);$i++){
    ...
    $_SESSION['tabla'].="<tr><td class='id' >".$i."</td><td >".$_SESSION['productos'][$i]->sku
    ...
    $i."' class='precios col-sm-9 text-left'  value='".$_SESSION['productos'][$i]->precio_compra."' readonly>
    </td><td >
    <input type='text' class='subtotal col-sm-9 text-left' name='txtsubtotal' id='txtsubtotal".count($_SESSION['productos'])."' readonly >
    </td></tr>";
  }
